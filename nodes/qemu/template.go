...
func (c *Client) Template(ctx context.Context, node string, vmid int, disk string) (string, error) {
	...
	var upid string
	if err := c.client.Create(ctx, "/nodes/"+node+"/qemu/"+strconv.Itoa(vmid)+"/template", &upid, p); err != nil {
		return "", err
	}
