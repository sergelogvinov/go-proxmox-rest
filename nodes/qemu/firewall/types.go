...
const (
	...
	// RuleTypeGroup references a (cluster-wide) security group's rule
	// set.
...
const (
	...
	// PolicyDrop silently drops traffic that reaches the default
	// policy.
	PolicyDrop Policy = "DROP"
)

// Rule describes a single firewall rule as returned by
// GET .../firewall/rules and GET .../firewall/rules/{pos}.
//
// Every field carries a `url` tag matching its wire name even though
// Rule is read-only and never passed to params.Encode: this package's
// decode path (internal/params.Decode, used by every GET) resolves each
// field's wire name from the `url` tag, not `json` — see decode.go's
// doc comment. The `json` tags are kept alongside for
// readability/documentation and are otherwise inert here (decode
// ignores them).
type Rule struct {
	// Pos is the rule's position within the guest's rule list. It
	// addresses the rule for Get/Update/Delete.
	...
	// RuleTypeGroup) the name of a cluster-wide security group to
	// apply.
	...
	// range, a CIDR, an alias ('alias-name') or an IP set
	// ('+ipset-name').
	...
	// Proto is the IP protocol, e.g. "tcp", "udp", or a protocol
	// number.
	Proto string `json:"proto,omitempty" url:"proto,omitempty"`
	// SPort restricts the TCP/UDP source port (a service name, a
	// number, or a range "start:end").
	SPort string `json:"sport,omitempty" url:"sport,omitempty"`
	// DPort restricts the TCP/UDP destination port, same syntax as
	// SPort.
	DPort string `json:"dport,omitempty" url:"dport,omitempty"`
	// IFace restricts the rule to one of the guest's network
	// interfaces, e.g. "net0".
	...
	// IPVersion is the IP version the rule was resolved against (4 or
	// 6); set by Proxmox, not settable.
	IPVersion int `json:"ipversion,omitempty" url:"ipversion,omitempty"`
	// Digest is the configuration digest, usable with
	// RuleOptions.Digest to guard concurrent updates.
	Digest string `json:"digest,omitempty" url:"digest,omitempty"`
}

// RuleOptions holds the write parameters shared by rule creation
// (POST .../firewall/rules) and rule updates
// (PUT .../firewall/rules/{pos}).
//
// Type and Action are required by Create. Proxmox does not echo the
// created rule back (POST responds with null data) and does not report
// the position it was assigned, so after Create use List to find the
// new rule — it is appended at the end unless Pos is set to insert it
// elsewhere.
//
// Pointer fields are always sent when non-nil (even zero/empty), which
// is how Update expresses "clear this field"; Create simply sends
// whatever is set. Pos, MoveTo, Delete and Digest are meaningful to
// specific verbs only (see field docs).
type RuleOptions struct {
	// Type is the rule kind: "in", "out", "forward" or "group".
	// Required by Create; Proxmox also requires it to be resent on
	// every Update.
	...
	// Pos is, on Create, the position to insert the new rule at
	// (existing rules from that position on shift down); Proxmox
	// appends to the end when unset. Not meaningful on Update — use
	// MoveTo to reposition an existing rule instead.
	Pos *int `url:"pos"`
	...
	// in between (value from the corresponding Get). Update/Delete
	// only.
...
type Alias struct {
	...
	// CIDR is the network/IP the alias resolves to, e.g.
	// "10.0.0.0/24".
	...
	IPVersion int `json:"ipversion,omitempty" url:"ipversion,omitempty"`
	// Digest is the configuration digest, usable with
	// AliasOptions.Digest to guard concurrent updates.
	Digest string `json:"digest,omitempty" url:"digest,omitempty"`
}

// AliasOptions holds the write parameters shared by
// POST .../firewall/aliases (Create) and
// PUT .../firewall/aliases/{name} (Update).
//
// Name and CIDR are required by Create. Proxmox requires CIDR to be
// resent on every Update even when unchanged. Update is addressed by
// the alias's current name in the URL (see aliasesResource.Update); set
// Rename to give it a new name.
type AliasOptions struct {
	// Name is the alias identifier. Required by Create; on Update it
	// is ignored (the current name is already part of the URL) — use
	// Rename to change it.
...
// IPSet describes an IP set as returned by GET .../firewall/ipset. It
// carries only the set's own metadata; its members are accessed via
// Client.IPSet(node, vmid).Entries(name).
type IPSet struct {
	...
	Comment string `json:"comment,omitempty" url:"comment,omitempty"`
	// Digest is the configuration digest, usable with
	// IPSetOptions.Digest to guard concurrent updates.
	Digest string `json:"digest,omitempty" url:"digest,omitempty"`
}
...
// metadata: creation and rename/re-comment both go through this same
// POST, distinguished by the Rename field — see ipsetResource.Update.
type IPSetOptions struct {
	// Name is the IP set's target name: the name to create (Create),
	// or the new name (Update — set equal to the current name to
	// update Comment without renaming).
	Name string `url:"name"`
	// Comment is the IP set description.
	Comment *string `url:"comment"`
...
// GET .../firewall/ipset/{name} and GET .../firewall/ipset/{name}/{cidr}.
...
// Update (Update is addressed by the entry's current CIDR in the URL;
// set CIDR here only if it differs, but Proxmox documents it as
// required regardless).
type IPSetEntryOptions struct {
	// CIDR is the network/IP of this member. Required by both Create
	// and Update.
	CIDR string `url:"cidr"`
	// Comment is the member description.
	Comment *string `url:"comment"`
...
// Options describes a guest's firewall configuration exposed by
// GET/PUT .../firewall/options. A single type serves both reads and
// writes: every field is optional, Proxmox only includes a key in the
// GET response once it has been explicitly set, and PUT only changes
// fields that are non-zero (via the `url` tag) — to explicitly reset a
// field to its default, list it in Delete instead of trying to send a
// zero value.
type Options struct {
...
// Source/Dest fields, as returned by Client.Refs. Datacenter-, SDN-,
// and guest-scoped aliases/IP sets are all included, distinguished by
// Scope.
type Ref struct {
	...
	Name string `json:"name,omitempty" url:"name,omitempty"`
	// Ref is the value to use in a rule's Source/Dest field to
	// reference this alias or IP set (e.g. "+ipset-name" for an IP
	// set).
	Ref string `json:"ref,omitempty" url:"ref,omitempty"`
	// Scope is the reference's scope: "dc" (cluster-wide), "sdn", or
	// "guest" (this VM's own).
...
// LogEntry is a single firewall log line, as returned by Client.Log.
...
// *LogOptions (or the zero value) requests Proxmox's default window.
