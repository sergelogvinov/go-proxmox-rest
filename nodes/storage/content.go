...
func (r *contentResource) List(ctx context.Context, node, storageID string, opts *ContentListOptions) ([]Volume, error) {
	...
	var volumes []Volume
	if err := r.client.Get(ctx, "/nodes/"+node+"/storage/"+storageID+"/content", &volumes, p); err != nil {
		return nil, err
	}
...
// volume id.
func (r *contentResource) Get(ctx context.Context, node, storageID, volume string) (*Volume, error) {
	v := &Volume{}
	if err := r.client.Get(ctx, "/nodes/"+node+"/storage/"+storageID+"/content/"+volume, v, nil); err != nil {
		return nil, err
	}
...
// id.
func (r *contentResource) Create(ctx context.Context, node, storageID string, opts *CreateVolumeOptions) (string, error) {
	if opts == nil {
	...
	var volid string
	if err := r.client.Create(ctx, "/nodes/"+node+"/storage/"+storageID+"/content", &volid, p); err != nil {
		return "", err
	}
...
func (r *contentResource) Update(ctx context.Context, node, storageID, volume string, opts *UpdateVolumeOptions) error {
	p, err := params.Encode(opts)
	if err != nil {
		return err
	}

	return r.client.Update(ctx, "/nodes/"+node+"/storage/"+storageID+"/content/"+volume, nil, p)
}
...
func (r *contentResource) Delete(ctx context.Context, node, storageID, volume string, delay int) (string, error) {
	var p map[string]string
	if delay > 0 {
	...
	var upid string
	if err := r.client.Delete(ctx, "/nodes/"+node+"/storage/"+storageID+"/content/"+volume, &upid, p); err != nil {
		return "", err
	}
