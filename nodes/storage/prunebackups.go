...
func (r *pruneBackupsResource) DryRun(ctx context.Context, node, storageID string, opts *PruneOptions) ([]PruneEntry, error) {
	...
	if err := r.client.Get(ctx, "/nodes/"+node+"/storage/"+storageID+"/prunebackups", &entries, p); err != nil {
		return nil, err
	}
...
func (r *pruneBackupsResource) Delete(ctx context.Context, node, storageID string, opts *PruneOptions) (string, error) {
	...
	var upid string
	if err := r.client.Delete(ctx, "/nodes/"+node+"/storage/"+storageID+"/prunebackups", &upid, p); err != nil {
		return "", err
	}
