...
func (c *Client) Suspend(ctx context.Context, node string, vmid int) (string, error) {
	return c.action(ctx, node, vmid, "suspend", nil)
}

// Resume resumes a suspended container via
// POST /nodes/{node}/lxc/{vmid}/status/resume. Returns the resume task's
// UPID.
func (c *Client) Resume(ctx context.Context, node string, vmid int) (string, error) {
	return c.action(ctx, node, vmid, "resume", nil)
}
