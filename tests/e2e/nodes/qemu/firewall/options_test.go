...
// Package firewall_e2e exercises the per-guest QEMU firewall module
// (options, rules, aliases, IP sets, refs, log) against a live Proxmox
// VE cluster.
//
// Unlike every other nodes/qemu subresource, Proxmox's firewall config
...
// /nodes/{node}/qemu/{vmid}/firewall/options against a fresh fake
...
func TestFirewallOptionsGetUpdate(t *testing.T) {
	...
	if cfg.Node == "" {
		t.Skip("PVE_E2E_NODE is not set; skipping qemu firewall tests")
	}
...
// /nodes/{node}/qemu/{vmid}/firewall/refs, unfiltered and filtered by
...
// TestFirewallLog verifies GET /nodes/{node}/qemu/{vmid}/firewall/log
