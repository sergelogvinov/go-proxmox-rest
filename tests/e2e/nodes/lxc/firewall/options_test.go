...
func TestFirewallOptionsGetUpdate(t *testing.T) {
	...
	oc := client.Nodes(cfg.Node).LXC().Firewall().Options(vmid)
...
func TestFirewallOptionsValidation(t *testing.T) {
	...
	err := client.Nodes(cfg.Node).LXC().Firewall().Options(uniqueVMID()).Update(ctx, nil)
...
func TestFirewallRefs(t *testing.T) {
	...
	client := e2e.NewE2EClient(t, cfg)
	fw := client.Nodes(cfg.Node).LXC().Firewall()
	ctx := t.Context()
	vmid := uniqueVMID()
...
func TestFirewallLog(t *testing.T) {
	...
	_, err := client.Nodes(cfg.Node).LXC().Firewall().Log(ctx, uniqueVMID(), &firewall.LogOptions{Limit: 5})
