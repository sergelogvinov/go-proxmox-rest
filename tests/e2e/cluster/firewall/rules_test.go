...
// firewall module against a live Proxmox VE cluster:
//
//	list → get(absent) → create → list(find by comment) → get → update → get → delete → list
...
// uniquely-commented cluster-wide firewall rule.
//
// Unlike ha.Rule, firewall.Rule has no caller-assigned identifier: Proxmox
// addresses a rule purely by its position (Pos) within the list, which it
// assigns automatically on Create and does not report back (Create's
// response is null data). So the rule is first created with a unique
// Comment, then located by re-listing and matching that comment to learn
// its Pos before Get/Update/Delete can be used.
func TestFirewallRulesLifecycle(t *testing.T) {
	...
	rr := client.Cluster().Firewall().Rules()
	ctx := t.Context()

	comment := e2e.UniqueName(cfg.Prefix)

	// Cleanup registry: best-effort delete-by-comment-search even if the
	// test fails mid-way, unless the caller asked to keep resources for
	// debugging. Pos is not known up front, so the closure re-lists and
	// searches by comment at cleanup time.
	...
	// 5. update — mutate the comment and disable the rule. Type and Action
	// must be resent (Proxmox requires them on every update).
...
// findFirewallRuleByComment returns the first rule in list whose Comment
// matches comment.
