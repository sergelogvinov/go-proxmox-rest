...
// GET /cluster/ceph/status and GET /nodes/{node}/ceph/status — Proxmox's
// own documentation states the per-node endpoint's response is identical
// to the cluster-wide one, and both cluster/ceph and nodes/ceph re-export
// this shape as their own Status via a type alias (see this file's doc
// comment and docs/design.md §2).
//
// Proxmox itself declares no schema for either endpoint (its API viewer
// marks the return type as a bare "object"): it is the raw `ceph status`
// RADOS command output, merged with `ceph health detail` and, on older
// Ceph releases, a re-fetched mon/mgr map, plus a `blocks-restart` flag
// Proxmox adds to every firing health check. Only the fields that are
// stable across Ceph releases and commonly consumed are typed; the rest
// — health checks, mutes, individual monitor/manager entries, the CephFS
// and service maps, and in-progress operation events — are exposed as
// generic values so no information is lost to a shape this client does
// not pin down.
type Status struct {
